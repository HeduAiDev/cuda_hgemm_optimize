...
namespace tui {
    namespace runable {
        ...
        void diff_(T *ptr_a, T *ptr_b, int rows, int cols, float accuracy = 1e-3) {
            ...
            {
                float diff = std::abs(static_cast<float>(ptr_a[i] - ptr_b[i]));
                if (diff >= accuracy) {
        ...
        void diff(double *ptr_a, double *ptr_b, int rows, int cols, float accuracy) {
            diff_<double>(ptr_a, ptr_b, rows, cols, accuracy);
        }
        void diff(int *ptr_a, int *ptr_b, int rows, int cols, float accuracy) {
            diff_<int>(ptr_a, ptr_b, rows, cols, accuracy);
        }
        #ifdef __CUDA__
        void diff(half *ptr_a, half *ptr_b, int rows, int cols, float accuracy) {
