...
// 0.358662 ms, M=N=2048, K=1024
__global__ void mma_ldmatrix_kernel(half* __restrict__ A, half* __restrict__ B, half* __restrict__ C, int M, int N, int K) {
    int tid = threadIdx.x;
    int warp_id = tid / 32;
    int lane_id = tid % 32;
    ...
    #define smem_warpB_ptr (smem_B + offset_warp_ld_frag_b * ldm_blockB)
    ...
    for (int k = 0; k < K; k += BlockTileK) {
        ...
        for (int i = tid; i < BlockTileN * BlockTileK / float4_element_num; i += blockDim.x) {
            ...
            float4 buffer = reinterpret_cast<float4*>(blockB_ptr)[offset_ld2s_global_by * ldm_B_f4size + offset_ld2s_global_bx + k / float4_element_num];
...
gemm::base::GemmOutput mma_ldmatrix(half* A_ptr, half *B_ptr, half *C_ptr, int M, int N, int K, const int launch_times) {
    ...
    for (int i = 0; i < launch_times; i++) {
        t.start();
        mma_ldmatrix_kernel<<<grid, block>>>(A.devicePtr(), B.devicePtr(), C.devicePtr(), M, N, K);
        t.stop();
