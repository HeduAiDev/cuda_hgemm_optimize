...
gemm::base::GemmOutput mma_smem(half* A_ptr, half *B_ptr, half *C_ptr, int M, int N, int K, const int launch_times) {
    ...
    for (int i = 0; i < launch_times; i++) {
        t.start();
        mma_smem_kernel<<<grid, block>>>(A.devicePtr(), B.devicePtr(), C.devicePtr(), M, N, K);
        t.stop();
